...
- ✅ **Enterprise UI Widgets & Dialog System (W-048, v0.5.2)**: Complete UI widget system with draggable dialogs, complex form interactions, and accordion components. Features include `jPulse.UI.alertDialog`, `jPulse.UI.infoDialog`, and `jPulse.UI.confirmDialog` with enterprise-grade capabilities (nested dialogs, custom callbacks, dontClose flag), draggable dialog headers with viewport bounds, accordion component with mutual exclusion, enhanced HTML sanitization for form elements, and conditional file includes (`file.exists` helper). Moved collapsible to `jPulse.UI.collapsible` namespace with comprehensive test coverage (22 UI widget tests, 18 collapsible tests).
- ✅ **Site-Specific Coding & Styling Guidelines (W-047, v0.5.1)**: Complete site development guidelines with comprehensive documentation, template system, and demo implementation. Features include `site-*` CSS prefix convention for clear source identification, JavaScript extension pattern via `jPulse.site` namespace, template-based file generation (`site-common.css.tmpl` → `site-common.css`), automatic loading via framework header, interactive demo pages, and essential test coverage. Implements "don't make me think" principle for site developers with zero configuration required.
- ✅ **Site Override Architecture (W-014, v0.5.0)**: Complete site customization system enabling seamless framework updates while preserving site-specific modifications. Features include automatic file resolution priority (`site/webapp/` → `webapp/`), auto-discovery of site controllers and APIs, configuration merging (framework + site overrides), context extension system for templates, and protected paths safe from framework updates. Implements "don't make me think" principle with zero manual route registration.
