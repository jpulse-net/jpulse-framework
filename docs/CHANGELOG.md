...
**Commit:** `W-205, v1.7.9: auth: signup with email confirmation`

**Objective**: Confirm a new signup actually controls the email address they registered with, needed to close the OAuth pre-linking account-takeover W-198 flagged (an attacker who signs up first with a victim's real email could otherwise transparently inherit the victim's first SSO login to that address) — made optional via a new site-level policy setting, with "don't make me think" as the guiding UX principle for both site admins and end users.

**Summary**: New `appConfig.controller.user.emailVerification` policy (`'off'` / `'nag'` / `'required'`, default `'required'`, read live on every request via `UserModel.getEmailVerificationPolicy()` rather than cached at startup) controls whether a fresh signup must prove email ownership. `'required'` injects a new `email-verify` step into the existing W-109 multi-step login flow (`AuthController._getRequiredSteps()`) — a signup is auto-sent a verification email right after account creation and must complete it (a 6-digit code typed in-flow, or the mailed link) before the session completes; `'nag'` lets login through immediately and shows a dismissible toast with a resend link instead. Either credential completes verification — one shared token pair per pending verification, issued by new `UserModel.issueEmailVerification()` (bcrypt-hashed in Redis, link 24h TTL / code 30m TTL) and consumed by `verifyEmailByToken()`/`verifyEmailByCode()`. The mailed link (`GET /api/1/user/email-verify/confirm?token=...`, no auth required — the token proves identity on its own) can finish a login started on a *different* device/browser: the originating tab detects this via a new, dedicated `GET /api/1/auth/pending-status` endpoint, deliberately kept outside both the Node-level per-IP login rate limiter and nginx's stricter `login` zone, since a status poll guesses no secret and shouldn't share either budget with real credential submissions — found necessary only after manual testing showed the original design (sharing `POST /api/1/auth/login`) tripped `RATE_LIMITED` after a few minutes of background polling. Resend/verify attempts are separately rate-limited (Redis-backed, fail-open): 3 sends / 10 min and 5 verify attempts / 15 min, per account. New `emailVerified` (boolean) + `emailVerifiedAt` (date, nullable) fields on the user schema; `emailVerifiedAt: null` alongside `emailVerified: true` unambiguously means grandfathered (backfilled) rather than actually proven — only successful verification stamps a real date. A one-time, idempotent startup backfill (`UserModel.ensureIndexes()`) normalizes any pre-existing account with an absent `emailVerified` to `true`/`emailVerifiedAt: null`, replacing W-198's original "absent reads as verified" convention with an explicit, queryable state. **SMTP safety valve**: `'required'` transparently degrades to `'nag'` at runtime whenever `EmailController.isConfigured()` is false, so a not-yet-configured (or later-broken) mail server can never lock every new signup out; a loud, non-mutating warning is logged once at startup (`checkEmailVerificationSafety()`) so the gap isn't silent, and full enforcement resumes immediately once SMTP is configured, with no restart. Admin UI: a verified/unverified badge in `admin/users.shtml`'s email cell, an `emailVerified` checkbox plus a read-only "verified on" line in `admin/user-profile.shtml` (both hidden entirely when the policy is `'off'`); an admin changing a user's `email` resets `emailVerified`/`emailVerifiedAt` by default (an admin-typed address is a belief, not proof) unless the same request explicitly asserts `emailVerified: true` — sends an informational email (with a verify link) to the new address and a security alert to the old one, and the response's `emailVerifiedReset` flag lets the admin UI explain a checkbox that came back unchecked. User-facing: a verification status line in Settings (hidden when the policy is `'off'`), and a new shared `/auth/email-verify.shtml` page serving three contexts with one URL — mid-login (`pendingAuth`), authenticated self-service (`'nag'` mode), and the confirm link's own `?status=verified|expired|invalid` landing, with cross-device polling every 8s that stops after 3 consecutive failures and falls back to a manual "Check now" button.

Found and folded in beyond the original spec, all discovered during implementation or manual testing: **(1)** signup's hardcoded `preferences.language: 'en'` (unrelated pre-existing bug found while wiring recipient-language email delivery) — now honors the browser-negotiated language like every other new-session default. **(2)** New `EmailController.sendEmailFromTranslation(req, { user, key, context, to, cc, bcc, replyTo, from })` sends a whole email — envelope headers plus body — from one translation key, unix-mail style (`Subject: ...` header line(s), blank line, body), rendered in the recipient's language via new `i18n.translateForUser()`. This superseded an earlier, file-based `webapp/static/assets/email/*.tmpl` approach that turned out broken twice over: `webapp/static/` is served raw by nginx in production with no `.tmpl` filtering (framework template files would have been readable in prod), and the `templatePath` values passed to `PathResolver.resolveAsset()` were missing the required `assets/` prefix, so all three sends were silently failing (`TEMPLATE_ERROR`). `EmailController.ALLOWED_EMAIL_HEADERS` was broadened from `Subject`-only to the common envelope headers (`Subject`/`To`/`Cc`/`Bcc`/`Reply-To`/`From`, case-insensitive), each individually overridable via a matching `sendEmailFromTranslation()` option; header values are substituted then stripped of `\r`/`\n` before use, so a `{{token}}` context value can never inject a fake header. `EmailController`'s auto-derived-HTML branch (escape → linkify → `<br/>`) was removed entirely — emails are text-only unless explicit HTML is supplied. **(3)** Fixed a latent `i18n.js` bug: `_translate()`'s substitution fell back to the literal `{{token}}` placeholder for legitimately falsy values (e.g. an empty `firstName`); extracted a new `i18n.substitute()` helper with the corrected `p1 in context ? context[p1] : match` logic. **(4)** New `CommonUtils.isSafeRedirectUrl(req, url)` (server-side counterpart to `jPulse.url.isInternal()`) and `CommonUtils.maskEmail(email)` for the toast/resend UI; the mailed confirm link carries no `redirect` param itself — the eventual destination rides `session.pendingAuth.redirect`, captured once at the credentials step or SSO callback and re-validated immediately before the confirm route's own redirect. **(5)** Manual testing surfaced two more bugs after the cross-device-poll fix above: a same-session second tab/window (e.g. two tabs of the same browser profile) that completed login via the confirm link left the *first* tab's poll seeing no `pendingAuth` (already consumed) and reporting a misleading `NO_PENDING_AUTH` "please sign in again" — fixed by having `pendingStatus()` check `req.session.user?.isAuthenticated` first. **(6)** Also from manual testing: the MFA-not-enabled nag (and any other `onAuthGetWarnings` toast) never reached the user when login completed via a plain server redirect (`confirmEmailVerify()`, or the pre-existing `completeExternalAuth()` for OAuth/LDAP/SAML) — the client's only toast queue is `sessionStorage`-based and unreachable from a server-issued `302`. Fixed generically with new `CommonUtils.appendToastsToUrl(url, warnings)` (base64-encoded `toasts` query param, no-op when empty), consumed by `jpulse-common.js`'s `dom.ready()` bootstrap. **(7)** That fix still didn't surface the nag, for two further reasons: `webapp/routes.js`'s `GET /` handler discarded the `toasts` query string along with everything else on its hard-coded redirect to `/home/` (now forwards its incoming query string); and the *waiting* tab's `pendingStatus()` poll is a separate HTTP request from the confirm-link tab's `_completeLoginSession()` call, so it never saw that call's `warnings` return value — `_completeLoginSession()` now also stashes non-empty warnings onto `req.session.pendingWarnings` (self-cleaning), which the already-authenticated shortcut in `pendingStatus()` drains and returns.

Verified via the full unit/integration suite (121 suites / 3014 tests via `npx jest --runInBand`), no regressions; extensive manual end-to-end testing across multiple accounts/browsers/incognito windows covering: signup → verify by code, signup → verify by mailed link (same browser and cross-device), resend + rate limiting, expired/invalid link landing states, the SMTP-not-configured degrade-to-nag path, the `'nag'` toast and settings-page status line, admin badge/checkbox/reset behavior (including the security alert to the old address and notice to the new one), and the same-session multi-tab / cross-device polling fixes above (MFA nag now shown correctly in all observed scenarios).

**Key features**:
- `appConfig.controller.user.emailVerification`: `'off'` / `'nag'` / `'required'` (default), evaluated live via `UserModel.getEmailVerificationPolicy()` — no restart needed for a config change or for the SMTP safety-valve degrade to take effect
- Dual verification path (6-digit code or mailed link, either completes it), with a dedicated, rate-limiter-exempt `GET /api/1/auth/pending-status` for cross-device completion detection
- SMTP safety valve: `'required'` never locks out signups when mail isn't configured, degrading transparently to `'nag'` with a one-time startup warning
- One-time idempotent grandfathering backfill for accounts predating this feature; admin badge/checkbox/reset UI with notice + security-alert emails on an admin-driven email change
- New `EmailController.sendEmailFromTranslation()` — whole email (envelope + body) from one recipient-language translation key, unix-mail style
- Generic post-login warning delivery across server-issued redirects (`CommonUtils.appendToastsToUrl()`), fixing a previously-silent MFA nag loss on the OAuth/LDAP/SAML and email-confirm-link paths alike

**Files changed**:
- `webapp/model/user.js`: `emailVerified`/`emailVerifiedAt` schema fields; `issueEmailVerification()`, `verifyEmailByToken()`, `verifyEmailByCode()`, `_completeEmailVerification()`, `sendEmailChangedNotice()`/`sendEmailChangedAlert()`, `getEmailVerificationPolicy()`; `ensureIndexes()` absent-field backfill
- `webapp/controller/auth.js`: `_getRequiredSteps()` email-verify step injection; `login()` handling for the `email-verify` step and resend; nag toast + `pendingWarnings` stash in `_completeLoginSession()`; new `pendingStatus()` (own endpoint) plus shared `_getExpectedStep()`/`_pendingAuthTimeoutMs()` helpers; `completeExternalAuth()` now carries warnings across its redirect
- `webapp/controller/user.js`: `confirmEmailVerify()`, `emailVerify()`, `emailVerifySend()`; `signup()` auto-send; `update()` admin email-change reset (+ `emailVerifiedReset` response flag)
- `webapp/controller/email.js`: `sendEmailFromTranslation()`, broadened `ALLOWED_EMAIL_HEADERS`, `_parseEmailMessage()` header parsing + header-injection guard; derived-HTML branch removed
- `webapp/controller/markdown.js`: unrelated pre-existing bug surfaced by the `i18n.substitute()` fix (was passing `baseDir` instead of `namespace` into its own error message)
- `webapp/utils/bootstrap.js`: `checkEmailVerificationSafety()` (non-mutating startup warning)
- `webapp/utils/common.js`: `isSafeRedirectUrl()`, `maskEmail()`, `appendToastsToUrl()`
- `webapp/utils/i18n.js`: `substitute()` (extracted, falsy-value bug fixed); `translateForUser()`
- `webapp/routes.js`: new email-verify + pending-status routes; `GET /`'s redirect now forwards its query string
- `webapp/app.conf`: `controller.user.emailVerification` (default `'required'`)
- `webapp/translations/en.conf`, `webapp/translations/de.conf`: `model.user.emailVerify`/`emailChangedNotice`/`emailChangedAlert` (unix-mail-style messages); `controller.auth.emailVerify*`/`controller.user.emailVerify.*`/view strings
- `webapp/view/auth/email-verify.shtml` (new): shared mid-login/self-service/confirm-link page with cross-device polling
- `webapp/view/jpulse-common.js`: `dom.ready()` bootstrap decodes/shows/strips a `toasts` URL query param via shared `showQueuedToasts()`
- `webapp/view/user/settings.tmpl`, `webapp/view/user/index.shtml`: verification status line; `webapp/view/admin/users.shtml`: verified/unverified badge; `webapp/view/admin/user-profile.shtml`: checkbox + "verified on" line
- `templates/deploy/nginx.prod.conf`: comment clarifying `/api/1/auth/pending-status` deliberately stays out of the stricter `login` zone

**Documentation**:
- `docs/security-and-auth.md`, `docs/api-reference.md`: new "Email Verification" sections (policy modes, SMTP safety valve, grandfathering, admin email-change reset, endpoints, rate limits)
- `docs/sending-email.md`: new "Email from a Translation Key" section
- `docs/plugins/plugin-hooks.md`: note on `completeExternalAuth()` now carrying `onAuthGetWarnings` toasts across its redirect
- `docs/dev/design/W-205-auth-email-confirmation.md` (new): full design doc, including deviations found during implementation and manual testing
- `docs/dev/work-items.md`: W-205 objectives/prerequisites/rationale/features/deliverables
- `README.md`, `docs/README.md`: Latest Release Highlights — v1.7.9 / W-205
- `docs/CHANGELOG.md`: this section
...
**Commit:** `W-203, v1.7.7: infrastructure: fix trustProxy - documented app.conf setting had zero implementation, making req.protocol/req.ip/req.secure unreliable behind a reverse proxy`

**Objective**: Make Express's `trust proxy` setting actually work. `docs/deployment.md`/`templates/deploy/README.md` documented a `trustProxy: true` app.conf setting for reverse-proxy deployments, but `webapp/app.js` never called `app.set('trust proxy', ...)` anywhere — a documented-but-nonfunctional setting.

**Summary**: Discovered while fixing W-197 Gap 9 (`plugins/auth-oauth`'s `computeRedirectUri()` sent Google an `http://` redirect_uri on `bubblemap.net`'s first live login attempt, a `req.protocol` behind-reverse-proxy bug) — traced one level further per the "root cause before fixes" debugging discipline, since the plugin's own bug looked like it might be a symptom of a framework-level gap rather than a plugin-only mistake, which it was. Confirmed via repo-wide search: `webapp/app.js` never called `app.set('trust proxy', ...)` anywhere, and `trustProxy` wasn't defined in any config schema either — not validated, not surfaced in the admin Config UI, and setting it in `app.conf` did precisely nothing. Net effect on any deployment behind a TLS-terminating reverse proxy (the framework's own reference `templates/deploy/nginx.prod.conf`, which does correctly set `X-Forwarded-Proto` on every location block, and is the *standard* `npx jpulse setup` path, not a rare one): Express's `req.protocol` always resolved to `http`, `req.secure` was always `false`, and `req.ip` returned the proxy's own address rather than the real client's — all silently, with no error or warning anywhere. `webapp/utils/common.js` already worked around the `req.ip` half of this by reading `X-Forwarded-For`/`X-Real-IP` directly instead of trusting `req.ip` — the framework had an established, working pattern for IP, just never extended it (or real `trust proxy` support) to protocol/secure. Fixed by wiring a new `middleware.trustProxy` config setting straight through to `app.set('trust proxy', ...)`, so any value Express itself accepts works (boolean, hop count, or a trusted IP/CIDR string or array), not just a plain boolean. Config key is `middleware.trustProxy`, not the previously-documented `app.trustProxy` — `app` is reserved for jPulse/site branding metadata, while `middleware` already holds every other Express-level setting (`cors`, `session`, `bodyParser`, `setHeaders`); nothing could have relied on the old `app.trustProxy` location's behavior, since it never did anything. Default value is split by layer: `webapp/app.conf` (framework core default, ships with every install) stays `false` — matching Express's own default, since the framework itself doesn't know how a given site will be deployed and must not silently start trusting client-supplied headers on a directly-exposed server; `templates/webapp/app.conf.tmpl` (used by `npx jpulse configure` for every new site) and `site/webapp/app.conf.tmpl` (the manual-copy starter template) both default to `true` instead, since the framework's standard, documented deployment path always terminates TLS at nginx — so every newly configured site is correct out of the box, while the framework package itself remains safe-by-default for the unknown case. `plugins/auth-oauth`'s own `getRequestProtocol()` (W-197 v1.0.3) is deliberately left in place rather than simplified to bare `req.protocol` — the plugin's `jpulseVersion` floor (`>=1.7.6`) predates this fix, so older-but-still-compatible framework installs (or any site admin who hasn't set `middleware.trustProxy`) still need the plugin's own header-reading fallback; complementary defense-in-depth, not redundant. Verified via the full unit/integration suite (123 suites / 3009 tests via `npx jest --runInBand` — a plain parallel `npx jest` run shows spurious cross-test failures unrelated to this change, a pre-existing test-isolation artifact of this repo's suite, not a regression), independently confirmed via `bin/test-all.js` (3038 passed, 0 failed, 0 skipped).

**Key features**:
- `middleware.trustProxy` is a real, working `app.conf` setting — any deployment behind a TLS-terminating reverse proxy gets correct `req.protocol`/`req.secure`/`req.ip`/`req.hostname` once set, with no other code changes needed anywhere in the framework or its plugins
- Every newly configured site (`npx jpulse configure`) is correct out of the box for the standard nginx-fronted deployment path, with an inline comment explaining when to turn it off

**Files changed**:
- `webapp/app.conf`: added `middleware.trustProxy: false` (framework default) with an inline comment explaining the setting and its accepted value types
- `webapp/app.js`: `app.set('trust proxy', appConfig.middleware.trustProxy)`, called immediately after `express()` creation, before any middleware/route that might read those properties
- `templates/webapp/app.conf.tmpl`, `site/webapp/app.conf.tmpl`, `site/webapp/app.conf` (this repo's own dogfooding site, which also runs `jpulse-net-prod` behind nginx): all three set `middleware.trustProxy: true` with an explanatory comment

**Documentation**:
- `docs/deployment.md`: fixed a broken `app.trustProxy`/`app.port` example in "Custom Web Server Setup" (didn't even match `app.conf`'s real structure) to the correct `middleware.trustProxy: true`; added a callout in "Deployment Architecture" explaining why the standard nginx path needs this and that `npx jpulse configure` sets it by default
...
**Commit:** `W-200, v1.7.4: plugins: add onPluginConfigBeforeSave hook so plugin config saves can transform/encrypt values before persistence`

**Objective**: Let a plugin's `type: "custom"` config field (W-194) participate in the page's single generic "Save Changes" action for anything that needs server-side processing before persistence (most importantly: encrypting a secret), instead of being forced into its own fully separate save path/button as the only safe option today.

**Summary**: Discovered while building W-197's auth-oauth plugin — its "Identity Providers" custom-rendered field (a CRUD table of OAuth provider configs, each with a Client Secret) hit this gap directly and shipped a real usability bug because of it. Today, `type: "custom"` fields are documented to expose exactly `{ container, value, onChange, schema, config, disabled }` to the renderer, with `onChange(v)` as the only channel back to the framework and "framework persists it as JSON" verbatim — no field-specific processing of any kind. The page's own generic "Save Changes" button hits a fully plugin-agnostic endpoint (`PluginController.updateConfig()`) that only does schema-shape validation before one whole-document `PluginModel.upsert()` — it has no hook for "transform/encrypt this one field's value before it touches storage." Net effect: any custom renderer whose value contains something that must never be written as-is (e.g. a plaintext secret) cannot safely rely on the generic Save button at all — it must own a fully separate, plugin-specific write path to get a chance to intercept and transform the value server-side first, forcing a confusing two-Save-buttons-on-one-page UX (auth-oauth's v1.0.0 shipped a stopgap warning-banner fix, not a structural one). Fixed by a new `onPluginConfigBeforeSave` hook, fitting the framework's existing plugin hook system (W-105) rather than bolting a new concept onto the `type: "custom"` client contract. This required a new `HookManager.executeForPlugin(hookName, pluginName, context)` method rather than the existing `execute()`/`executeWithCancel()`/`executeFirst()`, all three of which broadcast to every plugin registered for the hook and swallow (log-and-continue) any thrown error — exactly wrong for this use case, since a swallowed encryption failure would leave `configData` still containing the raw secret for `PluginModel.upsert()` to happily persist in the clear. `executeForPlugin()` instead scopes to a single plugin's handlers and does **not** catch errors — they propagate straight to the caller, which is the abort mechanism. `oldConfig` in the hook context (already fetched unconditionally by `updateConfig()` for change-logging, just passed through) is what makes the established "leave the field blank to keep the existing secret" convention actually implementable — without it a handler can't tell "admin left this blank, keep the old encrypted ref" apart from "admin wants to clear it." `PluginController.updateConfig()` calls the hook right after the existing `oldConfig` fetch and before `PluginModel.upsert()`; a thrown error aborts the whole save with a 400 `CONFIG_SAVE_REJECTED` response, showing the admin the thrown message verbatim (plugin authors must throw a user-facing, non-sensitive message, never a raw crypto/library error). Migrating `auth-oauth` onto the new hook (to retire its W-197 stopgap banner and collapse its two Save buttons into one) is an explicit, optional follow-up — not part of this release, since its existing dedicated-endpoint pattern is already correct and secure. Verified via the full unit suite (119 suites / 2905 tests, including 6 new `hook-manager.test.js` tests and 4 new `plugin-controller.test.js` tests) and manually via a live `npm start` + Admin → Plugins session using a throwaway test plugin registering a handler that throws on one input value and mutates on another — both the rejection path (save blocked, thrown message shown, nothing persisted) and the mutation path (persisted value is the handler-mutated one, not the raw submitted value) confirmed working as designed.

**Key features**:
- `HookManager.executeForPlugin(hookName, pluginName, context)` — filters registered handlers down to just the ones registered by `pluginName` (not a broadcast to every plugin), and does **not** catch handler errors — they propagate straight to the caller, which is the abort mechanism
- `onPluginConfigBeforeSave` registered in `getAvailableHooks()`: `context: '{ req, pluginName, configData, oldConfig }'`, `canModify: true`, `canCancel: true` — its description explicitly calls out that "cancel" here means "handler throws," not "handler returns `false`" (unlike every other `canCancel: true` hook in the registry)
- `PluginController.updateConfig()` calls the hook after the existing `oldConfig` fetch and before `PluginModel.upsert()`; a thrown error aborts the whole save with `CommonUtils.sendError(req, res, 400, hookError.message, 'CONFIG_SAVE_REJECTED')`
- New "Plugin Config Hooks" documentation section (`plugin-hooks.md`) and a worked "Encrypting a Plugin Config Secret" example demonstrating the handler pattern, the "throw to abort" contract, and the `oldConfig`-based "leave blank to keep existing secret" convention

**Files changed**:
- `webapp/utils/hook-manager.js`:
  - added `executeForPlugin(hookName, pluginName, context)` (single-plugin scope, propagates errors instead of swallowing them)
  - registered `onPluginConfigBeforeSave` in `getAvailableHooks()`
  - bumped the stale "Phase 8: ... (13 hooks total)" docblock comment (already stale at 14 since W-195 added `onAuthGetLoginProviders`) to 15
- `webapp/controller/plugin.js`:
  - `updateConfig()` now invokes the hook right after the existing `oldConfig` fetch and before `PluginModel.upsert()`; a thrown error maps to a 400 `CONFIG_SAVE_REJECTED` response
- `webapp/tests/unit/utils/hook-manager.test.js` (6 new tests):
  - `executeForPlugin()` single-plugin scoping, error propagation (not swallowed/logged-and-continued), stop-at-throwing-handler, no-handlers-registered no-op case, and the hook's `getAvailableHooks()` registration shape
- `webapp/tests/unit/controller/plugin-controller.test.js` (4 new tests):
  - `updateConfig()`'s new hook call — correct `{ req, pluginName, configData, oldConfig }` context, happy path (handler-mutated `configData` is what's passed to `PluginModel.upsert()`), rejection path (handler throws → 400 `CONFIG_SAVE_REJECTED`, `PluginModel.upsert()` never called), and the hook still firing even when the plugin declares no config schema

**Documentation**:
- `docs/plugins/plugin-hooks.md` — new "Plugin Config Hooks" section using the `%DYNAMIC{plugins-hooks-list-table namespace="onPluginConfig"}%` mechanism, plus a new "Encrypting a Plugin Config Secret" worked example in "Common Use Cases"
- `docs/plugins/plugin-api-reference.md` — cross-referenced the new hook from the `type: "custom"` section
- `docs/plugins/creating-plugins.md` — "Available Hooks" bullet list gains `**Plugin Config (1):** onPluginConfigBeforeSave`
- `docs/dev/work-items.md` — W-200 objective/features/deliverables/test-verify
- `README.md`, `docs/README.md` — Latest Release Highlights — v1.7.4 / W-200
- `docs/CHANGELOG.md` — this section
...
**Commit:** `W-189, v1.6.46: jPulse.UI: schema-form generator, async loadOptions, onInit lifecycle hook, showWhen conditional visibility, jpSelect/jpCombo input types; plugin-config consolidation`

**Objective**: Make the schema-form generator (`jPulse.UI.tabs.renderTabsAndPanelsFromSchema` + `_renderSchemaBlockFields`) the single source of truth for dynamic forms across `/admin/config.shtml` and `/admin/plugin-config.shtml`. Expose `jpSelect` / `jpCombo` widgets as schema `inputType`s, add a declarative async option loader (`loadOptions`), a generic lifecycle hook (`onInit`), and a first-class conditional-visibility attribute (`showWhen`). Consolidate the previously-separate plugin-config renderer onto the unified surface.

**Summary**: New `jPulse.schemaForm` namespace exposes a handler registry (`register` / `unregister` / `resolve`) that lets `plugin.json` (which can't carry functions) reference loadOptions / onInit handlers by name. The unified renderer gains nine new `inputType` cases (`jpSelect`, `jpCombo`, `radio`, `checkboxGroup`, `help`, `separator`, `email`, `url`, `tel`, plus `multiselect` as an alias rewritten internally to `jpSelect` + `multiple`) and reads flat widget-tuning keys (`search`, `searchPlaceholder`, `selectAll`, `allowCustom`, `multiple`) directly from the field def — no nested `widgetOptions` wrapper, matching the existing `slider` precedent (`min` / `max` / `step` / `default` / `suffix` are flat). `loadOptions` and `onInit` accept either a function (JS schemas) or a registry-name string (`plugin.json`), resolved via `jPulse.schemaForm._resolveHandler`. The post-render pipeline runs `loadOptions → option swap (or error state) → onInit → widget init` per field, in parallel via `Promise.allSettled`; the returned tabs handle now exposes a `ready` Promise that resolves after every deferred field settles. `showWhen` accepts `equals` / `notEquals` (scalar or array) leaves and `all` / `any` compounds; the framework wires up delegated `change` + `input` listeners on the form root, toggles `.jp-schema-field-hidden`, and removes/restores the `required` attribute via `data-jp-required-cache` so HTML5 validation auto-skips hidden fields. `pluginSchemaToBlocks` adapter normalizes legacy plugin.json `type` keys to the unified `(type, inputType)` pair via a documented table; the `flattenBlockValues` inverse flattens block-keyed save data back to plugin's flat shape.

**Key features**:
- **`jPulse.schemaForm` namespace** — `register` / `unregister` / `resolve` (handler registry); `applyOptions` / `setLoading` / `setError` (render helpers); `evalShowWhen` / `setupShowWhen` (conditional visibility); `runPostRender` (public alias for the post-render pipeline); `pluginSchemaToBlocks` / `flattenBlockValues` (plugin.json adapter).
- **New `inputType` cases** — `jpSelect`, `jpCombo`, `radio`, `checkboxGroup`, `help`, `separator`, `email`, `url`, `tel` (+ `multiselect` alias rewritten to `jpSelect`+`multiple`). Existing `text` / `password` / `number` / `slider` / `select` / `textarea` / `tagInput` / `checkbox` / `button` unchanged.
- **Flat widget tuning keys** — `search`, `searchPlaceholder`, `selectAll`, `allowCustom`, `multiple` live as top-level keys on the field def (no `widgetOptions` wrapper). Emitted on the rendered `<select>` as `data-jp-search` / `data-jp-search-placeholder` / `data-jp-select-all` / `data-jp-allow-custom`. `initAll` reads them via `_widgetOptionsFromDataAttrs(el)`.
- **Async `loadOptions(ctx)`** — function or registry-name string; while pending wrapper has `.jp-form-input-loading` (spinner); rejection sets `.jp-schema-field-error` with inline message and original options remain as fallback.
- **Generic `onInit(ctx)`** — runs after `loadOptions` settles and before widget init; mutate `ctx.widgetOptions` (passed to `jpSelect.init` / `jpCombo.init`) for advanced widget callbacks (`onOptionPreview`, `onCustomValue`, etc.) not exposed as flat keys.
- **`ready` Promise** — `renderTabsAndPanelsFromSchema(...)` handle now has a `.ready` Promise that resolves after every deferred field has been processed and `setupShowWhen` has run.
- **Declarative `showWhen`** — `{ field, equals/notEquals, all/any }`; bare field paths resolve to the current block, dotted paths are taken as-is; cross-block references work both ways. Hidden fields preserve their value (predictable HTML-form behavior); HTML5 `required` is auto-skipped (cached on `data-jp-required-cache`); widget instance preserved (no teardown).
- **Plugin-config consolidation** — `webapp/view/admin/plugin-config.shtml` rewritten to use `pluginSchemaToBlocks` + the unified renderer; the previous custom field-renderer (`renderField` / `renderTextInput` / `renderSelect` / etc., ~200 lines) deleted. Single-tab plugins still render flat (no tabs UI); multi-tab plugins go through `renderTabsAndPanelsFromSchema`.
- **`pluginSchemaToBlocks` normalization table** — legacy plugin.json `type` → unified `(type, inputType)`: text/password/email/url/tel/textarea/select/radio/jpSelect/jpCombo → `string` + `inputType=<same>`; number → `number`; slider → `number`+`inputType:slider`; boolean/checkbox → `boolean`; multiselect → `array`+`inputType:multiselect`; checkbox-group → `array`+`inputType:checkboxGroup`; tagInput → `array`+`inputType:tagInput`; help/separator → `inputType:<same>`; hidden → skipped.
- **`required` attribute support** — renderer emits HTML5 `required` for text/select/textarea/number inputs and the first radio in a group when `fieldDef.required === true`; auto-skipped for hidden fields (see `showWhen`).
- **`radio` and `checkboxGroup` setAllValues/getAllValues** — `setAllValues` checks/unchecks the right inputs by `name` (radio: scalar value; checkboxGroup: array value); `getAllValues` reads radio's checked input value, checkboxGroup's array of checked values.
- **CSS additions** — `.jp-form-input-loading` (spinner via `::after` + `@keyframes jp-schema-field-spin`); `.jp-schema-field-error` (red border via `--jp-theme-color-danger`); `.jp-schema-field-error-msg`; `.jp-schema-field-hidden` (`display: none`); `.jp-form-radio-group` (+ `-horizontal` modifier); `.jp-form-checkbox-group`. All colors via `--jp-theme-*` variables.
- **`type` vs `inputType` clarification** — orthogonal: `type` for value coercion (`getFormData`), `inputType` for rendering (`_renderSchemaBlockFields`); both keys explicit in JS schemas; plugin.json's single-`type` style normalized via the adapter.
- **Polish & regression fixes (same release)** — `inputType: 'help'` renders with `.jp-schema-help` instead of `.jp-alert` (toast positioning) for correct inline layout. After tab `register()` moves panels, `_runSchemaPostRender` scans the tab root (`rootEl`) so deferred `loadOptions` fields are found and loading clears. `_runSchemaPostRender` yields one microtask then runs `setupShowWhen` before awaiting `loadOptions`, removing a visible flash for initially hidden fields. `enum` / `options` arrays whose items are `{ value, label }` objects map correctly (no `[object Object]` labels). `getFormData` / `setFormData` skip display-only `help`, `separator`, and `button` paths. **jpCombo**: text input `click` opens the dropdown; `openDropdown` keeps focus on the text input (dialog focus-trap compatibility); modal `_trapFocus` bypass / Tab extension includes jpCombo portal; option commit uses `mousedown`; `focusout` ignores `relatedTarget === null` for portaled options; arrow button `mousedown` prevents focus for ARIA. **Plugin-config** `saveConfiguration()` calls `form.reportValidity()` before `getFormData`. **Example**: `webapp/view/jpulse-examples/ui-widgets.shtml` custom dialog demonstrates schema tabs, `loadOptions`, `showWhen`, `reportValidity`, and `flattenBlockValues`.
- **Tests** — extended `jpulse-ui-tabs-schema.test.js` with `_resolveInputType` (multiselect alias, type inference), `_buildSelectOptionsHtml` (scalar/array selection), `_resolveShowWhenPaths` (path resolution + compound recursion), and one branch per new inputType + flat tuning keys + `loadOptions` defer + `showWhen` JSON emission + `required` attr. New `jpulse-schema-form.test.js` covers the registry, `evalShowWhen` (equals scalar/array, notEquals, all/any, missing field), `_collectShowWhenDeps`, `setupShowWhen` integration (initial visibility, change-event re-eval, required-attr toggle, idempotent re-call), and helper API. New `jpulse-schema-form-pipeline.test.js` exercises `_runSchemaPostRender` end-to-end in JSDOM (`loadOptions`, `onInit`, `ready`, `showWhen`, tab-root regression). New `plugin-config-renderer.test.js` covers the normalization table (one row per legacy type), tab grouping, default values, `type:hidden` skip, tab-label-to-block-key normalization, `flattenBlockValues`, and a render→setFormData→getFormData→flatten round-trip. New `plugin-config-view.test.js` asserts `reportValidity` before save. `jpcombo.test.js` updated for defer-init filter, `mousedown` selection, and relatedTarget guard. Full unit suite: **86 suites, 2359 tests passing** (at release prep).

**Files changed**:
- `webapp/view/jpulse-common.js` — new `jPulse.schemaForm` namespace; `_renderSchemaBlockFields` refactor (helpers + new inputTypes + flat tuning keys + data-jp-* emission); `renderTabsAndPanelsFromSchema` post-render pipeline + `ready` Promise; `setAllValues` / `getAllValues` radio + checkboxGroup support; `initAll` `:not([data-jp-defer-init])` filter; `required` attr emission; jpCombo + dialog focus-trap fixes; display-only field walk for form data
- `webapp/view/jpulse-common.css` — schema-form loading / error / hidden / radio-group / checkbox-group / `.jp-schema-help` styles
- `webapp/view/admin/plugin-config.shtml` — rewritten to use `pluginSchemaToBlocks` + unified renderer; `reportValidity` before save; ~250 lines of bespoke field-rendering code deleted
- `webapp/view/jpulse-examples/ui-widgets.shtml` — schema-driven custom dialog example (W-189)
- `webapp/tests/unit/utils/jpulse-ui-tabs-schema.test.js` — extended with W-189 cases (renderer + helpers)
- `webapp/tests/unit/utils/jpulse-schema-form.test.js` — new (registry + showWhen runtime + helper API)
- `webapp/tests/unit/utils/jpulse-schema-form-pipeline.test.js` — new (post-render pipeline + tab-root + display-only fields)
- `webapp/tests/unit/utils/plugin-config-renderer.test.js` — new (adapter + normalization table + integration round-trip)
- `webapp/tests/unit/view/plugin-config-view.test.js` — new (`reportValidity` before `getFormData`)
- `webapp/tests/unit/controller/jpcombo.test.js` — updated for `:not([data-jp-defer-init])`, `mousedown` behavior, `relatedTarget` guard

**Documentation**:
- `docs/jpulse-ui-reference.md` — new "Schema-Driven Form Generator (W-189)" section: type vs inputType callout, full inputType table (including `.jp-schema-help` for `help`), flat widget tuning keys table, `loadOptions` / `onInit` lifecycle (function and string forms), `ctx` shape, `showWhen` (operators, paths, hidden-field behavior), `jPulse.schemaForm.*` API surface, plugin.json normalization table
- `docs/front-end-development.md` — extended "Schema-driven config forms" section with "Async options & lifecycle", "Conditional visibility", and "Plugin.json schemas" subsections
- `docs/genai-instructions.md` — schema-form authoring rules in Core Directives (use schema-driven flow over hand-rolling, flat tuning keys, loadOptions/onInit/showWhen guidance, type vs inputType)
- `README.md`, `docs/README.md` — Latest Release Highlights — v1.6.46 / W-189
- `docs/CHANGELOG.md` — this section
- `webapp/view/jpulse-examples/ui-widgets.shtml` — live + quoted schema-dialog example aligned with W-189 patterns
