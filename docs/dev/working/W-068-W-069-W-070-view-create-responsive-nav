...
window.jPulseSiteNavigation = {
    ...
    admin: {
        ...
        icon: '⚙️',  // Optional: emoji or filename (e.g., 'config.svg')
        pages: {
            dashboard: {
...
- **Conditional visibility**: Use `{{#if user.isAuthenticated}}` for auth-based sections
...
1. **Emoji**: Single character (e.g., `icon: '📦'` or `icon: '🔄'`)
...
2. **Image file**: Known extensions (.jpg, .png, .svg, .gif)
   ...
   - Sized to 16×16px (or scaled to font size)
...
- **Auto** (default): Overlay on mobile (<768px), reflow on desktop (≥768px)
...
- **2nd+ levels**: Hover over item with sub-pages → submenu appears to the right
...
- **Parent fallback**: `/admin/foo.shtml` not in nav → highlights parent `/admin/`
...
- **URL matching**:
  - Exact match: `/admin/config.shtml` → Home > Admin > Config
  - Parent fallback: `/admin/foo.shtml` not in nav → Home > Admin > (current page)
...
- **Conditional rendering**: Auth-based sections with `{{#if user.isAuthenticated}}`
