# jPulse Framework v1.7.14 Deployment Guide for %SITE_NAME%
...
3. Confirm `middleware.trustProxy: true` in `site/webapp/app.conf` (already set by default -
   only relevant here if you changed or removed it)
...
- **`app.conf`**: Committed to the repo; site name, domain, dev/prod deployment sections, no secrets. Generated by `npx jpulse configure`.
- **`app-secret.conf`**: Gitignored; per-environment secrets and `deployment.mode`. Omit for local dev — `npm start` uses `deployment.mode: 'dev'` from `app.conf`. For production, create from template and set real session secret, DB auth, Redis passwords.

See the [Configuration Files](#configuration-files) table below and [Installation — Site Configuration](https://%DOMAIN_NAME%/jpulse/installation#site-configuration) for the three-layer merge (framework → app.conf → app-secret.conf).
...
#### Configuration Files
| File | Committed | Purpose | Generated By |
|------|-----------|---------|--------------|
| `site/webapp/app.conf` | ✅ Yes | Site config (no secrets) | `npx jpulse configure` |
| `site/webapp/app-secret.conf` | ❌ No (gitignored) | Secrets + deployment mode | `npx jpulse configure` |
| `.env` | ❌ No (gitignored) | Shell/PM2 environment variables | `npx jpulse configure` |
| `deploy/nginx.prod.conf` | optional | Web server config | `npx jpulse configure` |
| `deploy/ecosystem.*.cjs` | optional | PM2 process management | `npx jpulse configure` |
